...
export class APath {
    ...
    direct_compare(template: APath, thresh = 0.01): boolean {
        ...
        // return template.points.every(point => Math.min(...this.points.map(p => point.minus(p).mag_sq())) <= thresh * thresh) &&
        //     this.points.every(point => Math.min(...template.points.map(p => point.minus(p).mag_sq())) <= thresh * thresh)
    }
}
...
export class Font {
    ...
    constructor(
        ...
        this.map = new Map(Object.entries(map_in).map(([char, { paths }]) => ([APath.conormalize(paths), char])))
    ...
    find_letters(s: APath[], thresh = 0.01): FontMatchedLetter[] {
        const skips: (null | number)[] = s.map(() => null)
    ...
    static from_cfg(max_size: number, map_in: Record<string, [Partial<FontLetterInfo>, ...Array<[Array<[number, number]>, boolean]>]>): Font {
        return new Font(
            max_size,
            Object.fromEntries(
                Object.entries(map_in)
                    .map(([ch, [info_partial, ...v]]) => (
                        [
                            ch,
                            {
                                paths: v.map(([p, loop]) => new APath(p.map(([x, y]) => new Vec2(x, y)), loop, false)),
